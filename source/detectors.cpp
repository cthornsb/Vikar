...
unsigned int TestDetSetup(Planar *bar_array, unsigned int num_bars, unsigned int num_trials, bool WriteRXN_, double fwhm_/*=0.0*/, double angle_/*=0.0*/){	
	...
	while(count < num_trials){
		...
		if(WriteRXN_){ 
			REACTIONdata.Zero();
			REACTIONdata.Append(0.0, offset.axis[0], offset.axis[1], offset.axis[2], temp_ray.axis[0], temp_ray.axis[1], temp_ray.axis[2], 0.0); 
		}
...
// fwhm_ (m) allows the use of a gaussian particle "source". If fwhm_ == 0.0, a point source is used
// angle_ (rad) allows the rotation of the particle source about the y-axis
unsigned int TestDetSetup(Planar *bar_array, Kindeux *kind_, unsigned int num_bars, unsigned int num_trials, double beamE_,
						  unsigned int num_bins_, double start_, double stop_, double fwhm_/*=0.0*/, double angle_/*=0.0*/){
	if(!bar_array || !kind_ || !kind_->IsInit()){ return 0; }
	double tempx, tempy, tempz;
	unsigned int count, total;
	unsigned int bar;
	Vector3 flight_path;
	...
	std::ofstream hists("mcarlo.hist");
	if(!hists.good()){ return 0; }
	
	double low_angles[num_bins_], high_angles[num_bins_];
	double step = (stop_ - start_) / num_bins_;
	unsigned int bin_counts[num_bins_+2];
	
	for(unsigned int i = 0; i < num_bins_; i++){
		low_angles[i] = start_ + i*step;
		high_angles[i] = low_angles[i] + step;
		bin_counts[i] = 0;
	}
	bin_counts[num_bins_] = 0; // underflow
	bin_counts[num_bins_+1] = 0; // overflow
		...
		for(bar = 0; bar < num_bars; bar++){
			...
			if(eject_hit && recoil_hit){
				...
				if(com_angle < low_angles[0]){ bin_counts[num_bins_]++; } // underflow event
					...
					for(unsigned int i = 0; i < num_bins_; i++){
						if(com_angle >= low_angles[i] && com_angle < high_angles[i]){
							bin_counts[i]++;
							break;
						}
					}
	...
	hists << "########################\n";
	hists << "# Underflow: " << bin_counts[num_bins_] << std::endl;
	hists << "# Overflow: " << bin_counts[num_bins_+1] << std::endl;
	hists << "# Total Events: " << total << std::endl;
	hists << "# Good Events: " << count << std::endl;
	hists << "########################\n";
	hists << "# bin\tlow\thigh\tcount\n";
	for(unsigned int i = 0; i < num_bins_; i++){
		hists << i << "\t" << low_angles[i] << "\t" << high_angles[i] << "\t" << bin_counts[i] << std::endl;
	}
